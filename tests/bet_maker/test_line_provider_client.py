...
Retry-factory behaviour. No HTTP server needed — we test the predicate
against fabricated httpx exceptions and test the factory against a
counter-style coroutine.
"""
...
class TestLineProviderUnavailable:
    """LineProviderUnavailable carries a reason attribute."""

    def test_reason_attribute_is_set(self) -> None:
        """.reason holds the string passed in __init__."""
    ...
    def test_str_returns_reason(self) -> None:
        """str(exc) yields the reason for traceback hygiene."""
...
class TestIsRetryable:
    ...
    def test_4xx_http_status_error_is_not_retryable(self, status_code: int) -> None:
        """HTTPStatusError 4xx -> False (LP contract responses)."""
    ...
    def test_value_error_is_not_retryable(self) -> None:
        """Arbitrary non-http exceptions do not trigger retry."""
        assert _is_retryable(ValueError("bad payload")) is False


class TestMakeRetryDecorator:
    """Factory returns a tenacity decorator with the right wiring."""
    ...
    async def test_retries_then_succeeds(self) -> None:
        """Retryable exception once, success second time -> succeeds with no raise."""
        counter = {"calls": 0}
    ...
    async def test_exhaustion_reraises_original(self) -> None:
        """reraise=True: after `attempts` retries the original exc propagates."""
        counter = {"calls": 0}
    ...
    async def test_non_retryable_propagates_immediately(self) -> None:
        """A 422 HTTPStatusError is NOT retried — exception fires on first call."""
        counter = {"calls": 0}
