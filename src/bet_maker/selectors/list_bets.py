...
async def list_bets(session: AsyncSession) -> list[BetRead]:
    ...
    GET /bets ordered by created_at DESC. No pagination, no filtering
    at test-task scale. Pure read -- no UoW, session is one-shot via
    get_session DI.

    BetRead.model_validate(row, from_attributes=True) inside the
    iteration — caller receives DTOs, never ORM instances.
    """
