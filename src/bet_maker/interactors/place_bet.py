...
class EventNotBettable(Exception):  # noqa: N818
    ...
    Route layer catches and maps to HTTPException(422,
    detail=f"event {event_id} is not bettable: {e.reason}").

    Three exact reason strings -- "event not found", "deadline passed",
...
async def place_bet(
    ...
    *,
    ...
    Validate (event_lookup snapshot exists, deadline in future, state ==
    NEW) THEN insert. Validation MUST happen BEFORE entering the UoW
    context -- failed validation must not trigger any DB transaction
    (verified by test_no_db_write_on_validation_fail).

    BetRead.model_validate(bet, from_attributes=True) is called INSIDE
    `async with uow:` -- after flush+refresh, before commit. This loads
    server_default fields (created_at, updated_at) under an open session,
    eliminating MissingGreenlet on attribute access from the route.
    """
