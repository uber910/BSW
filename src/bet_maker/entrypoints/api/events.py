...
bet-maker /events delegates to list_active_events selector which calls LP
via the singleton httpx.AsyncClient. On LineProviderUnavailable → 503 with
static detail string. No TTL cache. Empty list → 200 + [].
"""
...
async def get_events(http_client: LineProviderHttpClientDep) -> list[EventRead]:
    ...
    LineProviderUnavailable (upstream LP unreachable after retry).
