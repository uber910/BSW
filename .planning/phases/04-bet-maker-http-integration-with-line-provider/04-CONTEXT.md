...
- **D-18 [informational]:** **`test_health.py` не меняется** в P4. /health на текущем этапе пингует только PG (P5 расширит на RMQ); HTTP-клиент к LP в health-чек не добавляем — line-provider down не должен ронять bet-maker /health (read-side endpoint GET /events просто вернёт 503 для запросов, /bets и /bet/{id} продолжают работать). _Informational: this is a no-action decision; no plan needs to cite it because nothing is changed._
